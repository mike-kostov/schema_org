defmodule SchemaOrg.CompleteDataFeed do
  @moduledoc "A [[CompleteDataFeed]] is a [[DataFeed]] whose standard representation includes content for every item currently in the feed.\n\nThis is the equivalent of Atom's element as defined in Feed Paging and Archiving [RFC 5005](https://tools.ietf.org/html/rfc5005), for example (and as defined for Atom), when using data from a feed that represents a collection of items that varies over time (e.g. \"Top Twenty Records\") there is no need to have newer entries mixed in alongside older, obsolete entries. By marking this feed as a CompleteDataFeed, old entries can be safely discarded when the feed is refreshed, since we can assume the feed has provided descriptions for all current items."
  ...
  @doc false
  def __schema_org__, do: %{type: "CompleteDataFeed", json_keys: @json_keys}

  @doc "Builds an empty `SchemaOrg.CompleteDataFeed` struct."
  ...
  @doc "Sets the Schema.org `catalog` property.\n\nA data catalog which contains this dataset."
  def catalog(%__MODULE__{} = thing, value) do
    %{thing | catalog: value}
  end
  ...
  @doc "Sets the Schema.org `dataFeedElement` property.\n\nAn item within a data feed. Data feeds may have many elements."
  def data_feed_element(%__MODULE__{} = thing, value) do
    %{thing | data_feed_element: value}
  end

  @doc "Sets the Schema.org `datasetTimeInterval` property.\n\nThe range of temporal applicability of a dataset, e.g. for a 2011 census dataset, the year 2011 (in ISO 8601 time interval format)."
  def dataset_time_interval(%__MODULE__{} = thing, value) do
    %{thing | dataset_time_interval: value}
  end
  ...
  @doc "Sets the Schema.org `distribution` property.\n\nA downloadable form of this dataset, at a specific location, in a specific format. This property can be repeated if different variations are available. There is no expectation that different downloadable distributions must contain exactly equivalent information (see also [DCAT](https://www.w3.org/TR/vocab-dcat-3/#Class:Distribution) on this point). Different distributions might include or exclude different subsets of the entire dataset, for example."
  def distribution(%__MODULE__{} = thing, value) do
    %{thing | distribution: value}
  end
  ...
  @doc "Sets the Schema.org `includedDataCatalog` property.\n\nA data catalog which contains this dataset (this property was previously 'catalog', preferred name is now 'includedInDataCatalog')."
  def included_data_catalog(%__MODULE__{} = thing, value) do
    %{thing | included_data_catalog: value}
  end

  @doc "Sets the Schema.org `includedInDataCatalog` property.\n\nA data catalog which contains this dataset."
  def included_in_data_catalog(%__MODULE__{} = thing, value) do
    %{thing | included_in_data_catalog: value}
  end
